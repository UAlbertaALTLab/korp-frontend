  ...
  constructor(scope, timeout, utils, location, kwicDownload) {
    ...
    const massageData = function (hitArray) {
      ...
      for (let i = 0; i < hitArray.length; i++) {
        ...
        const currentStruct = {};
        for (const i in _.range(0, hitContext.tokens.length)) {
          ...
          for (const structItem of wd.structs.open || []) {
            ...
            for (const [key, val] of _.concat([[structKey, ``]], attrs)) {
          ...
          for (const structItem of wd.structs.close || []) {
...
korpApp.directive(`statsResultCtrl`, () => ({
  controller($scope, $location, backend, searches, $rootScope) {
    ...
    s.getGeoAttributes = function (corpora) {
      let attrs = {};
      for (const corpus of settings.corpusListing.subsetFactory(corpora).selected) {
    ...
    s.showMap = async function () {
      ...
      const cqpExprs = {};
      for (const rowIx of selectedRows) {
        if (rowIx === 0) {
          continue;
        }
      ...
      const mapPromise = backend.requestMapData(cqpExpr, cqpExprs, within, selectedAttribute);
      await Promise.all([
        mapPromise,
              ...
              if (type === `set` && attrVal.length > 1) {
                ...
                _.map(attrVal, function (val) {
                  ...
                  if (variants.length === 0) {
                    for (const idx of _.range(0, parts.length - 1)) {
                      variants.push([]);
                    }
                  }
                  for (const idx of _.range(1, parts.length)) {
